...
class SettingController extends Controller
{
    ...
    // dw notification
    public function dwNotification(Request $request)
    {
        $request->validate([
            'dw_notification_enabled' => 'required|numeric',
            'dw_notification_min_interval' => 'required|numeric',
            'dw_notification_max_interval' => 'required|numeric',
            'dw_notification_script' => 'required',
        ]);

        // make sure min is greater than greater than or equal to 1 and max is greater than min
        if ($request->dw_notification_min_interval < 1) {
            return response()->json(validationError('Minimum interval must be greater than or equal to 1'), 422);
        }

        if ($request->dw_notification_max_interval < $request->dw_notification_min_interval) {
            return response()->json(validationError('Maximum interval must be greater than minimum interval'), 422);
        }

        updateSite([
            'dw_notification_enabled' => $request->dw_notification_enabled,
            'dw_notification_min_interval' => $request->dw_notification_min_interval,
            'dw_notification_max_interval' => $request->dw_notification_max_interval,
            'dw_notification' => json_encode($request->dw_notification_script)
        ]);

        cache()->forget('dw-data');

        return response()->json(['message' => 'DW Notification settings updated successfully']);
    }
}
